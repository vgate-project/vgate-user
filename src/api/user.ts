...
import type { User, UserDashboard } from '@/types/api'

export const apiUser = {
  // GET /user/dashboard — one call bundling the home page + global badges
  // (profile, hourly traffic, recent orders, nodes, announcements, unread
  // tickets, telegram status, pending order). Replaces several separate calls.
  dashboard: () => http.get<UserDashboard>('/user/dashboard'),
  ...
  // GET /user/subscribe?type=raw,base64 — several formats in one request,
  // returned as a map keyed by token (e.g. { raw: "...", base64: "..." }).
  subscribeMulti: (formats: string[]) =>
    http.get<Record<string, string>>('/user/subscribe', {
      params: { type: formats.join(',') },
    }),

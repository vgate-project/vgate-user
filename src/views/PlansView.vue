...
import type { Plan, PlanPrice } from '@/types/api'
import { OrderKindPlan } from '@/types/api'
...
function selectedPriceFor(plan: Plan): PlanPrice | undefined {
  ...
  return plan.prices?.find((pr) => pr.period === id) || plan.prices?.[0]
}

// Build a short, left-aligned summary of the plan being purchased for the
// confirm dialog. Content comes from our own plan data, not user input, so
// rendering it as HTML is safe.
function planConfirmHTML(plan: Plan, price: PlanPrice): string {
  return (
    `<div style="text-align:left;line-height:1.9">` +
    `<div><b>Plan:</b> ${plan.name}</div>` +
    `<div><b>Billing period:</b> ${periodLabel(price)} (${price.duration_days} days)</div>` +
    `<div><b>Price:</b> ${formatPrice(price.price)}</div>` +
    `</div>`
  )
}

// ConfirmPurchase shows the selected plan and only proceeds (returning true)
...
async function doBuyPlan(plan: Plan, price: PlanPrice, platform: string) {
  ...
  try {
    ...
    if (data.paid) {
      ...
      pollUntilPaid(data.order.id, 'Payment successful. Plan is now active.')
    } else {
...
async function doSwitchPlan(plan: Plan, price: PlanPrice, platform: string) {
  buyingId.value = 'switch:' + plan.id + ':' + price.period
  try {
    ...
    const { data } = await apiChangePlan.changePlan({ plan_id: plan.id, plan_price_id: price.period, platform })
    if (data.paid) {
      ElMessage.success(`Switched to ${plan.name}. Paid with wallet balance: ${formatPrice(data.wallet_used_cents ?? 0)} (remaining ${formatPrice(data.wallet_remaining_cents ?? 0)}).`)
      await dashboard.refresh()
      return
    }
    if (data.pay_url || data.pay_mode === 'iap') {
      ...
      if (data.order) pollUntilPaid(data.order.id, `Switched to ${plan.name}.`)
    } else {
      ElMessage.warning('No payment link returned.')
    }
  } catch {
...
onMounted(async () => {
  try {
    const [plansRes] = await Promise.all([apiPlan.list(), loadPaymentMethods()])
    plans.value = plansRes.data
    plans.value.forEach((p) => {
      if (p.prices && p.prices.length) selectedPrice.value[p.id] = p.prices[0].period
    })
    await dashboard.refresh()
  } finally {
    loading.value = false
...
<template>
  <div v-loading="loading">
    <h2 style="margin: 0 0 16px">Plans</h2>
        ...
        <el-card shadow="hover" class="plan-card">
          ...
          <div v-if="plan.prices && plan.prices.length" class="price-options">
            <div
              ...
              :key="pr.period"
              class="price-row"
              :class="{ active: selectedPrice[plan.id] === pr.period }"
              @click="selectedPrice[plan.id] = pr.period"
            >
          ...
          <el-button
            type="primary"
            style="width: 100%"
            :loading="buyingId === (plan.id === currentPlanId ? 'plan:' : 'switch:') + plan.id + ':' + (selectedPriceFor(plan)?.period ?? '')"
